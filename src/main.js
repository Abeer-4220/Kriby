...
import { makeScoreCard } from "./scoreBox";
...
k.scene("main", async () => {
  ...
  const map = k.add([k.pos(200, -30), k.scale(SCALE_FACTOR)]);
  ...
  clouds.onUpdate(() => {
    clouds.move(clouds.speed, 0);
    if (clouds.pos.x > 250) {
      clouds.pos.x = -450; // put the clouds far back so it scrolls again through the level
    }
  });
  ...
  platforms.onUpdate(() => {
    ...
    if (platforms.pos.x < -490) {
      ...
      platforms.speed += 20; // progressively increase speed
    }
  });
